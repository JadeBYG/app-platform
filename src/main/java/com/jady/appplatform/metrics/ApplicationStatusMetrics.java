...
public class ApplicationStatusMetrics {
    ...
    private void registerGauge(MeterRegistry registry, String status) {
        Gauge.builder("app_application_status_size", statusCounts, m -> m.getOrDefault(status, 0.0))
                .tag("status", status)
                .register(registry);
    }
